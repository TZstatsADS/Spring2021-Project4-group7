...
$Z_i$ are a subset of covariates $X_i$ with sample average $\bar{Z}$, and $\tau$ is an estimate for ATE.
...
ggplot(data=thre, aes(x=thresholds, y=LD_ATE)) +
...
ggplot(data=thre, aes(x=thresholds, y=HD_ATE))+
...
colnames(high) = c("ATE", "Absolute Error", "Run Time")
# stratification
...
colnames(low) = c("ATE", "Absolute Error", "Run Time")
# stratification
...
```{r graph_high, fig.width=12, fig.height=5}
...
```{r graph_low, fig.width=12, fig.height=5}
...
In conclusion, we explored the three causal inference algorithms (Regression Estimate, Stratification,Weighted Regression with PS is based on L2 penalized logistic regression) on two data sets (low dimensional/ high dimensional) and computed the estimated ATE for each model. We evaluated the algorithms performance by calculating the absolute differences between the true ATE and estimated ATE and compare the computational efficiency by comparing the system run time.
